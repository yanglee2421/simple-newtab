export class ObjectURLStore {
  subscribeCountMap: WeakMap<Blob, number>;
  objectURLMap: WeakMap<Blob, string>;
  listensers: Set<() => void>;

  constructor() {
    this.subscribeCountMap = new WeakMap();
    this.objectURLMap = new WeakMap();
    this.listensers = new Set();
  }

  subscribe(blob: Blob, callbackFn: () => void) {
    this.listensers.add(callbackFn);
  ...
  unsubscribe(blob: Blob, callbackFn: () => void) {
    this.listensers.delete(callbackFn);
